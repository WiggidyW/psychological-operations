// AUTO-GENERATED by x-api-spec/codegen.py — do not edit by hand.
// Source: x-api-spec/openapi.json (sha256 dd4e81f817babcd5...)

//! HTTP call helpers for /2/users/{id}/followed_lists/{list_id}.
#[allow(unused_imports)]
use crate::x::http::Http;
#[allow(unused_imports)]
use crate::x::Error;
#[allow(unused_imports)]
use reqwest::Method;

/// DELETE /2/users/{id}/followed_lists/{list_id}
pub async fn delete(
    http: &Http,
    req: &super::delete::Request,
) -> Result<super::delete::Response, Error> {
    let path = format!("users/{}/followed_lists/{}", urlencoding::encode(&req.id.to_string()), urlencoding::encode(&req.list_id.to_string()));
    http.send::<_, ()>(Method::DELETE, &path, None).await
}

