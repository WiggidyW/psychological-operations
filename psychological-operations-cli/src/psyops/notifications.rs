...
#[derive(Subcommand)]
pub enum Commands {
    /// Get all per-psyop notifications, or one by index
    Get {
        name: String,
        index: Option<usize>,
    },
    /// Add a notification target to this psyop (JSON string)
    Add {
        name: String,
        json: String,
    },
    /// Remove a notification target by index from this psyop
    Del {
        name: String,
        index: usize,
    },
    ...
    pub fn handle(self) -> Result<crate::Output, crate::error::Error> {
        match self {
            ...
            Commands::Add { name, json } => {
                ...
                cfg.psyops.entry(name).or_default().notifications.push(parsed);
            ...
            Commands::Del { name, index } => {
                let mut cfg = crate::config::load();
                {
                    let entry = cfg.psyops.get_mut(&name)
                ...
                if cfg.psyops.get(&name).is_some_and(|e| e.is_empty()) {
