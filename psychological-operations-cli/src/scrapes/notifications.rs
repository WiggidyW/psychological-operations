...
#[derive(Subcommand)]
pub enum Commands {
    /// Get this scrape's per-name notifications. With `--commit <sha>` reads
    ...
    /// Add a notification target to this scrape. With `--commit <sha>` adds
    ...
    pub fn handle(self) -> Result<crate::Output, crate::error::Error> {
        match self {
            Commands::Get { name, index, commit } => {
                ...
                let list: Vec<Destination> = cfg.scrapes.get(&name).map(|o| match commit.as_deref() {
            ...
            Commands::Del { name, index, commit } => {
                let mut cfg = crate::config::load();
                {
                    let overrides = cfg.scrapes.get_mut(&name)
                    ...
                    let target = match commit.as_deref() {
                        Some(sha) => &mut overrides.commits.get_mut(sha)
                            .ok_or_else(|| crate::error::Error::Other(format!("no commit override \"{sha}\" for scrape \"{name}\"")))?
                            .notifications,
